...
OA, Kappa, AA, cm, classified_map = perform_classification(
    data, ground_truth, pixel_idx, idx_GKMI, test_size=0.8, classifier="RF"
)
...
patches = [
    mpatches.Patch(color=colors[i], label=classes[i].format(l=values[i]))
...
plt.axis("off")
plt.legend(
    handles=patches, bbox_to_anchor=(1.01, 1), loc=2, borderaxespad=0.0, fontsize=20
)
plt.savefig(
    os.path.join(output_dir, str(dataset) + "_classified.eps"),
    dpi=200,
    transparent=True,
    bbox_inches="tight",
)
...
io.savemat(
    os.path.join(output_dir, str(dataset) + "_results.mat"),
    {
