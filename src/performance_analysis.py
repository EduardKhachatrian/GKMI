...
def perform_classification(dataset, ground_truth, pixel_idx, idx_GKMI, 
    test_size=0.8, classifier='SVM'):
    ...
    for i in range(idx_GKMI.shape[0]):
        if classifier == 'SVM':
            classified_map[pixel_idx[i]] = svm(array_reshaped[pixel_idx[i],:][:,np.sort(idx_GKMI[i])], 
                                               Xtrain[:,np.sort(idx_GKMI[i])], ytrain)
        elif classifier == 'RF':
            classified_map[pixel_idx[i]] = rf(array_reshaped[pixel_idx[i],:][:,np.sort(idx_GKMI[i])], 
                                              Xtrain[:,np.sort(idx_GKMI[i])], ytrain)

        print ('%s / %s' % (counter, idx_GKMI.shape[0]))
        counter += 1

    _, _, y_pred, y_test = preparation_for_classification(classified_map, ground_truth, test_size)
...
def preparation_for_classification(dataset, ground_truth, test_size):
    ...
    # Split the dataset
    X_train, X_test, y_train, y_test = train_test_split(Xknown, yknown, test_size=test_size)
...
def rf(X, Xtrain, ytrain):
    ...
    clf = RandomForestClassifier(n_estimators=300, min_samples_leaf=2, max_depth=80, 
        min_samples_split=5, bootstrap=True, max_features='sqrt', n_jobs=-1)   
    clf.fit(Xtrain, ytrain)
    y_pred = clf.predict(X)
    return y_pred
